...
def createXml(outfile, ref, data):
    ...
    ColltnAmt.text = data['amt']
    FrDt.text = data['date']
    FrstColltnDt.text = data['date1']
    FnlColltnDt.text = data['date2']
    Nm_debtor_customer.text = data['name']
    Id_2_debtor.text = data['acc']
    Prtry_debtor.text = data['type']
    MmbId_debtor.text = MmbId2.text = data['ifsc']
    
    #indent and save the file
    ET.indent(tree, space='\t')
    tree.write(outfile, encoding='UTF-8', xml_declaration=True)

def makeXml(outfile, ref, data):
    Document = ET.Element('Document')
    MndtInitnReq = ET.SubElement(Document, 'MndtInitnReq')
